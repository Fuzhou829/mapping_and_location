 ...
 * @LastEditors: renjy
 * @LastEditTime: 2023-11-30 20:03:05
 */
...
bool Trilateration::_getGlobalPosFromMap(int map_id,
  ...
  std::map<int, int> high_risk_match;
  ...
  for (int i = 0; i < observation_info_size; i++) {
    for (int j = 0; j < observation_info_size; j++) {
      ...
      for (int k = 0; k < observation_info_size; k++) {
        ...
        if (!_checkTriangle(
            side_distance_ij, side_distance_ik, side_distance_jk)) {
            // SLAM_WARN("can not be a triangle %f %f %f",
            //   side_distance_ij, side_distance_ik, side_distance_jk);
            continue;
        }
        // 三边定位
        ...
        if (!match_id.count(landmark_ids[0]) &&
            !high_risk_match.count(landmark_ids[0])) {
          match_id.insert(std::make_pair(landmark_ids[0], i));
        } else {
          if (match_id.count(landmark_ids[0]) &&
              match_id.at(landmark_ids[0]) != i) {
            high_risk_match.insert(std::make_pair(landmark_ids[0], 2));
            match_id.erase(landmark_ids[0]);
          }
        }
        if (!match_id.count(landmark_ids[1]) &&
            !high_risk_match.count(landmark_ids[1])) {
          match_id.insert(std::make_pair(landmark_ids[1], j));
        } else {
          if (match_id.count(landmark_ids[1]) &&
              match_id.at(landmark_ids[1]) != j) {
            high_risk_match.insert(std::make_pair(landmark_ids[1], 2));
            match_id.erase(landmark_ids[1]);
          }
        }
        if (!match_id.count(landmark_ids[2]) &&
            !high_risk_match.count(landmark_ids[2])) {
          match_id.insert(std::make_pair(landmark_ids[2], k));
        } else {
          if (match_id.count(landmark_ids[2]) &&
              match_id.at(landmark_ids[2]) != k) {
            high_risk_match.insert(std::make_pair(landmark_ids[2], 2));
            match_id.erase(landmark_ids[2]);
          }
        }
        // is_finish_location =
