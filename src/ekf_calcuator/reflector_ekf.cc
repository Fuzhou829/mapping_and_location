 ...
 * @LastEditTime: 2023-11-30 18:37:46
 */
#include <iostream>
#include <cfloat>
#include <map>
...
void Observation::GetObsFromLadarData(const MAL::SensorMount& sensor_mount,
  ...
  if (landmark_config.recognition_type == MAL::RecognitionType::Cluster) {
    ...
    std::map<int, LandMark::Center> land_mark =
      land_mark_calcuator.GetLandMarkCenter(true);
    if (land_mark.empty()) return;

    for (auto iter = land_mark.begin(); iter != land_mark.end(); iter++) {
...
void ReflectorEkfCalcuator::HandleOdomData(const OdometerMessage& data) {
  ...
  get_odom_data_ = true;
  uint64_t last_time = last_odom_data_.mclDeltaPosition.mlTimestamp;
  if (odom_list_.size() > 200) {
    pthread_mutex_unlock(&odom_mutex_);
    _refreshOdom(last_time);
  } else {
    pthread_mutex_unlock(&odom_mutex_);
  }
}
  ...
  if (whole_match_size > 0) {
                    ...
                    (H_t * state_.sigma * H_t.transpose() + Q).inverse();
    {
      ...
      if (fabs(state_.mu(0) - x) > 0.1 || fabs(state_.mu(1) - y) > 0.1 ||
          fabs(SLAMMath::NormalizePITheta(state_.mu(2) - t)) > 0.087) {
        SLAM_WARN("update change %f %f %f",
                  state_.mu(0) - x, state_.mu(1) - y, state_.mu(2) - t);
        for (int i = 0; i < map_match_size; i++) {
        ...
        if (is_print) {
          internal_common::PrintEigen(K_t, "K_t");
          internal_common::PrintEigen(H_t, "H_t");
          internal_common::PrintEigen(state_.sigma, "state_.sigma");
        }
...
bool ReflectorEkfCalcuator::_isMatchToState(const Eigen::Vector2d& obs,
  ...
  float min_eu_dis = 0.1 + std::sin(theta_theshold) * 2 * SLAMMath::Dist(
  ...
  for (int j = 0; j < state_obs_size; ++j) {
    ...
    double map_ma_match_dis_threshold =
      std::sqrt(Eigen::Vector2d(min_eu_dis, min_eu_dis).transpose() *
                St.inverse() * Eigen::Vector2d(min_eu_dis, min_eu_dis));
    ...
    pridect_reflector(1) = -(global_reflector.x() - state_.mu(0)) *
                            ...
                            std::cos(state_.mu(2));
    // 预测值-实际观测
    Eigen::Vector2d delta_state = global_reflector - reflector;
    ...
    if (ma_dist < map_ma_match_dis_threshold &&
        ma_dist < min_distance) {
        min_distance = ma_dist;
        association_id = j;
        // if (eu_dist > 1.0) {
        //   SLAM_ERROR("association error, please check!!! "
        //           "association eu_dist %f ma_dist %f", eu_dist, ma_dist);
        // } else {
...
void ReflectorEkfCalcuator::_calOdomMessageSingleSteer(
  ...
  if (isnan(ldLineVel) || isnan(ldAngularVel) || isnan(ldTurnAng)) {
