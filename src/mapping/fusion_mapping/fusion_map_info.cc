 ...
 * @LastEditTime: 2023-12-01 10:57:03
 */
...
// // 两帧位姿之前的约束
// class TransformCostFunction {
//  public:
//   explicit TransformCostFunction(
...
//     T s_c = ceres::cos(start_pose[2]);
//     T s_s = ceres::sin(start_pose[2]);
//     T d_x = s_c * (end_pos[0] - start_pose[0]) +
//                    s_s * (end_pos[1] - start_pose[1]);
//     T d_y = -s_s * (end_pos[0] - start_pose[0]) +
//                    s_c * (end_pos[1] - start_pose[1]);
//     T d_t = SLAMMath::NormalizePITheta(end_pos[2] - start_pose[2]);
//     e[0] = ceres::abs(d_x - transform_[0]);
//     e[1] = ceres::abs(d_y - transform_[1]);
//     e[2] = ceres::abs(
//       SLAMMath::NormalizePITheta(d_t - transform_[2]));
//     return true;
//   }

//  private:
//   const std::array<double, 3> transform_;
// };
...
void FusionMap::CreateMapFromScans(
  ...
  //   // 0. 添加之前的约束信息
  //   Coordinate::Transform transform;
  //   transform.SetPose1InGlobal(Eigen::Vector3d(
  //     last_scan->GetSensorPose().GetX(),
  //     last_scan->GetSensorPose().GetY(),
  //     last_scan->GetSensorPose().GetHeading()));
  //   transform.SetPose2InGlobal(Eigen::Vector3d(
  //     current_scan->GetSensorPose().GetX(),
  //     current_scan->GetSensorPose().GetY(),
  //     current_scan->GetSensorPose().GetHeading()));
  //   Eigen::Vector3d relative_pos;
  //   transform.GetPose2InPose1(&relative_pos);
  //   ceres::CostFunction *cost_function_translate =
  //       new ceres::AutoDiffCostFunction<TransformCostFunction, 3, 3, 3>(
  //           new TransformCostFunction(
  //             std::array<double, 3>{
  //             relative_pos(0), relative_pos(1), relative_pos(2)}));
  //     problem.AddResidualBlock(cost_function_translate, NULL,
  //                               poses.at(last_scan->GetStateId()).data(),
  //                               poses.at(current_scan->GetStateId()).data());
  ...
  // int count = 0;
  // for (auto iter : poses) {
  //   problem.AddParameterBlock(poses.at(iter.first).data(), 3);
  //   if (count == 0) {
  //     problem.SetParameterBlockConstant(poses.at(iter.first).data());
  //   }
  //   count++;
  // }
  ...
  //           new LocalCostFunction(poses.at(iter.first)));
  //   // 设置残差项
  //   problem.AddResidualBlock(
  //       cost_function, NULL, poses.at(iter.first).data());
  // }
  ...
  //   SLAM_INFO("constraints %d %d %f %f", iter.last_id, iter.current_id,
  //                                         iter.delta_dis, iter.delta_theta);
  // }
  // // 处理问题
  ...
  //   SLAM_INFO("%f %f %f -> %f %f %f delta %f %f %f",
  //             current_scan->GetSensorPose().GetX(),
  //             current_scan->GetSensorPose().GetY(),
  //             current_scan->GetSensorPose().GetHeading(),
  //             pose[0], pose[1], pose[2],
  //             current_scan->GetSensorPose().GetX() - pose[0],
  //             current_scan->GetSensorPose().GetY() - pose[1],
  //             current_scan->GetSensorPose().GetHeading() - pose[2]);
