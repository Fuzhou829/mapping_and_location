 ...
 * @LastEditTime: 2023-11-17 22:18:25
 */
...
void ReflectorMapping::_startReflectorMapping(const std::string& map_name) {
  ...
  while (getline(ladar_file, ladar_line)) {
    ...
    if (last_time > ladar_message.mstruRadarHeaderData.mlTimeStamp) {
      SLAM_WARN("ladar time is error~~~");
      continue;
    }
    SLAM_INFO("ladar count %d", count++);
    // if (count > 1700) break;
    last_time = ladar_message.mstruRadarHeaderData.mlTimeStamp;
