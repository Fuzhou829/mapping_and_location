 ...
 * @LastEditTime: 2023-11-29 16:20:03
 * @Author: renjy
 */
...
bool MergeReflectors::_refleashAllSubmap() {
  ...
  int count = 3;
  while (count--) {
    for (auto iter : global_pose_) {
      if (iter.second.is_get_global_pose) continue;
      SubmapGlobalPose pos;
      if (_isLoop(sub_maps_[iter.first], &pos)) {
        pos.is_get_global_pose = true;
        global_pose_[iter.first] = pos;
        _runOptimizer();
        is_get_map = true;
      }
    }
...
void TrilateralMapping::StopMapping(const std::string& map_name) {
  ...
  float landmark_intensities =
    mapping_config_.land_mark_config.landmark_intensities;
  LandMark::Calcuator landmark_cal(mapping_config_.land_mark_config);
  landmark_cal.Init();
  while (getline(ladar_file, ladar_line)) {
    ...
    for (int i = 0;
      i < ladar_message.mstruSingleLayerData.mvPoints.size(); i++) {
      float intensity = ladar_message.mstruSingleLayerData.mvIntensities[i];
      if (intensity > landmark_intensities &&
          fabs(intensity - 176) > 0.001) {
          Eigen::Vector2d point =
            ladar_message.mstruSingleLayerData.mvPoints[i];
          landmark_cal.AddLandMarkCenter(
            LandMark::Point(point(0), point(1), intensity));
      }
    }
  }

  landmark_cal.SetFramesCount(count);
  std::map<int, LandMark::Center> landmark_info =
  landmark_cal.GetLandMarkCenter(true);
  std::vector<Eigen::Vector2d> centers;
  for (auto iter = landmark_info.begin();
      iter != landmark_info.end(); iter++) {
    if (!iter->second.is_cal_center) continue;
    centers.push_back(Eigen::Vector2d(iter->second.mx, iter->second.my));
  }
  merge_reflector_map.SetReflectorMapInfo(centers);
  std::map<int, std::map<int, Eigen::Vector2d>> result;
