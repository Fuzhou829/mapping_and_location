 ...
 * @LastEditors: renjy
 * @LastEditTime: 2023-11-30 19:38:25
 */
...
bool SubMapConstraint::ComputeSubMapConstraint(
  ...
  for (int i = submap_size - 2; i >= 0; i--) {
    ...
    if (i > submap_size - 3) {
      if (_calContraintForTimenearSubmap(submap_size - 1, i,
          last_submap_global_pos, current_submap_global_pos,
          reflectors_in_last_submap, reflectors_in_current_submap)) {
        // is_get_loop = true;
      } else {
        _isGetConstraintForLoopSubMap(new_submap, finished_submap[i]);
      }
    } else {
...
bool SubMapConstraint::_calContraintForTimenearSubmap(
  ...
  // if (trilater_cal.GetGlobalPos(
  // map_triangle, observation_triangle, &result)) {
...
bool SubMapConstraint::_isGetConstraintForLoopSubMap(
  ...
  // 闭环检测 -- z直接利用三边定位
  LandMark::Trilateration trilateration(config_.land_mark_config);
  trilateration.SetLandMarkMap(trilateration.Transform(new_submap_reflectors));
  Eigen::Vector3d pos_in_submap;
  if (!trilateration.IsGetGlobalPos(
      current_submap_reflectors, &pos_in_submap)) {
    SLAM_DEBUG("new_submap_id %d current_submap_id %d no constraint",
                new_submap_id, current_submap_id);
    return false;
  }

  Eigen::Vector3d current_in_global = current_submap->GetGlobalPose().first;

  Coordinate::Transform transform;
  transform.SetPose1InGlobal(new_submap->GetGlobalPose().first);
  transform.SetPose2InPose1(pos_in_submap);
  Eigen::Vector3d current_in_global_cal;
  transform.GetPose2InGlobal(&current_in_global_cal);
  float dis = SLAMMath::Dist(current_in_global_cal(0), current_in_global_cal(1),
                             current_in_global(0), current_in_global(1));
  if (dis > config_.land_mark_config.distance_for_loop) {
    SLAM_WARN("%d %d dis %f, %f %f %f -> %f %f %f",
              new_submap_id, current_submap_id, dis, current_in_global_cal(0),
              current_in_global_cal(1), current_in_global_cal(2),
              current_in_global(0), current_in_global(1), current_in_global(2));
    return false;
  }
  std::map<int, int> result;
  trilateration.GetMatchId(&result);
  for (auto iter : result) {
    constraints_.push_back(Constraint(new_submap_id, iter.first,
                          current_submap_id, iter.second,
                          config_.land_mark_config.error_threshold_for_loop));
  }
  SLAM_DEBUG("new_submap_id %d current_submap_id %d has constraint",
                new_submap_id, current_submap_id);
  return true;
...
bool SubMapConstraint::_isLoop(const std::vector<int>& obs_id,
  ...
  float delta_theta = fabs(SLAMMath::NormalizePITheta(
      corrected_new_submap_pos(2) - new_submap_pos(2)));
