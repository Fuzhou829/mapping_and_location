 ...
 * @LastEditTime: 2023-11-15 12:19:50
 */
...
: config_(config), thread_pool_(num_background_threads) {
  ...
  reflector_publisher_ = std::make_shared<
...
void ReflectorMapBuilder::DispatchLadarData(
  ...
  if (active_submaps_.size() == 1) {
    ...
    if (deal_size_submap0 ==
      ...
      finished_submaps_ = optimizer_->GetFinishedSubMaps();
  ...
  reflector_publisher_->DisPlayReflectorMapping(ladar_message,
    state.mu, obs.centers, active_submaps_[0]->GetSubMapId());
  return;
}
...
void ReflectorMapBuilder::SaveToFile(const std::string& map_name) {
  ...
  // for (const auto &submap : finished_submaps_) {
  //   for (const auto &local_center :
  //       submap->GetReflectorInGlobal().reflectors_) {
  //     LandMark::DbscanPoint tmp;
  //     tmp.x = local_center(0);
  //     tmp.y = local_center(1);
  //     tmp.clusterID = LandMark::UNCLASSIFIED;
  //     unclustered_centers.push_back(tmp);
  //     landmark_cal.AddLandMarkCenter(LandMark::Point(tmp.x, tmp.y));
  //   }
...
std::shared_ptr<Submap> ReflectorMapBuilder::_createdNewSubMap(
  ...
  if (!finished_submaps_.empty()) {
    // state.mu.head(3) =
    //   finished_submaps_[new_submap_ptr->GetSubMapId() - 2]
    //   ->GetFinishedPose().first;
    // state.sigma.block(0, 0, 3, 3) =
    //   finished_submaps_[new_submap_ptr->GetSubMapId() - 2]
    //   ->GetFinishedPose().second;
  }
  new_submap_ptr->InitReflectorEkf(state);
  new_submap_ptr->SetGlobalPose(state.mu.topRows(3));
  // 求解新子图到上一个子图坐标系下的坐标
