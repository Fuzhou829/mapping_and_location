 ...
 * @LastEditors: renjy
 * @LastEditTime: 2023-11-28 19:41:38
 */
  ...
  if (result->empty()) {
    ...
    for (int i = 0; i < map_json["landmarks_pos_list"].size(); i++) {
      sub_map.insert(std::make_pair(
        map_json["landmarks_pos_list"][i]["landmark_id"].asInt(),
...
void PrintEigen(const Eigen::MatrixXd& mat, std::string name) {
  for (int i = 0; i < mat.rows(); i++) {
    std::string data;
    for (int j = 0; j < mat.cols(); j++) {
      data += std::to_string(mat(i, j)) + " ";
    }
    SLAM_INFO("%s: %d --- %s", name.c_str(), i, data.c_str());
  }
  return;
}
