 ...
 * @LastEditTime: 2023-11-22 11:26:32
 */
/*
...
void AMCLLaser::SetModelLikelihoodField(double z_hit, double z_rand,
  ...
  map_update_cspace(this->map, max_occ_dist, weighted_areas);
}
...
void AMCLLaser::SetModelLikelihoodFieldProb(
  ...
  SLAM_INFO("use_weighted_areas: %d", use_weighted_areas_);
  if (use_weighted_areas_) {
    SLAM_INFO("likelihood map_data_file_path: %s/ale.smap", map_data_file_path_);
    weighted_areas = new WeightedAreas(map_data_file_path_ + "/ale.smap");
  }
  map_update_cspace(this->map, max_occ_dist, weighted_areas);
}
...
double AMCLLaser::BeamModel(AMCLLaserData *data, pf_sample_set_t *set) {
  ...
  for (j = 0; j < set->sample_count; j++) {
    ...
    for (i = 0; i < data->range_count; i += step) {
      ...
      obs_range = sqrt(data->ranges[i][0] * data->ranges[i][0] +
                       data->ranges[i][1] * data->ranges[i][1]);
      obs_bearing = std::atan2(data->ranges[i][1], data->ranges[i][0]);
...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data,
  ...
  // step = (data->range_count - 1) / (self->max_beams - 1);
  real_partilces = static_cast<float>(data->range_count) / (static_cast<float>(step));
  ...
  for (j = 0; j < set->sample_count; j++) {
    ...
    for (i = 0; i < data->range_count; i += step) {
      ...
      //     obs_range = data->ranges[i + k][0];
      //     obs_bearing = data->ranges[i + k][1];
      //   修改为点云！！！！！
      // obs_range = sqrt(data->ranges[i + k][0] * data->ranges[i + k][0] +
      //             data->ranges[i + k][1] * data->ranges[i + k][1]);
      // obs_bearing =
      //  std::atan2(data->ranges[i + k][1], data->ranges[i + k][0]);
