 ...
 * @LastEditTime: 2023-11-30 14:59:48
 * @Author: lcfc-desktop
 */
...
LocationManagerImpl::LocationManagerImpl(
  ...
  is_check_result_ = false;
  total_odom_  = 0.0;
  qr_distance_ = 0.0;
  ekf_for_amcl_ = std::make_shared<DataFusion::EKFCalcuator>(
                  config.ekf_config, "ekf_for_amcl_");
  ...
  reflector_location_ =
    std::make_shared<ReflectorLocation>(config);

  wan_ji_location_ =
    std::make_shared<WanJiLocation>(location_config_);
...
void *LocationManagerImpl::_reflectorLocationFunction(void *param) {
  ...
  while (1) {
    ...
    last_pos = pc->_refreshOdom(radar_data.mstruRadarMessage.
                     mstruRadarHeaderData.mlTimeStamp);
    ...
    pc->reflector_location_->HandleLaserData(
      radar_data, last_pos, is_can_do_location);
    std::map<LocationType, int> types =
            pc->location_data_fusion_cal_->GetLocationType();
    if (!pc->is_finish_global_location ||
        !types.count(LocationType::LandMarkLocation)) continue;
    if (pc->reflector_location_->GetCurrentPosition(&reflector_pose)) {
      if (pc->_isPosJump(last_pos, reflector_pose) && pc->is_move_) {
        if (is_get_jump) {
          jump_distance += SLAMMath::Dist(last_pos.mfX, last_pos.mfY,
                          last_ladar_pos.mfX, last_ladar_pos.mfY);
        }
        pc->reflector_current_pos_ = last_pos;
      } else {
        jump_distance = 0.0;
        pc->reflector_current_pos_ = reflector_pose;
        SLAM_DEBUG("reflector_pose %f %f %f",
            reflector_pose.mfX, reflector_pose.mfY, reflector_pose.mfTheta);
  ...
  while (1) {
    if (pc->is_amcl_deal_radar_data_ && pc->have_init_pose_ && pc->have_map_) {
        ...
        std::map<LocationType, int> types =
            pc->location_data_fusion_cal_->GetLocationType();
        if (pc->is_finish_global_location &&
            !types.count(LocationType::AMCL)) continue;
        if (pc->is_finish_global_location &&
        ...
        //   }
        // }
        bool is_need_update = pc->is_finish_global_location &&
            last_pos.mlTimestamp == radar_time;
        bool is_can_do_location = pc->is_move_ && pc->is_finish_global_location;
        internal_common::RunTime run_time;
        pc->grid_location_->HandleLaserData(
          radar_data, last_odom_pos, is_can_do_location);
        ...
        if (is_need_amcl_location && pc->is_finish_global_location &&
          ...
          if (pc->grid_location_->GetCurrentPosition(&now_amcl_pos)) {
            ...
            SLAM_INFO("amcl run time %f ms jump_distance %f delta to now %ld",
                      run_time.ElapsedMicroSecond() / 1000.0f, jump_distance,
                      gomros::common::GetCurrentTime_us() -
                      now_amcl_pos.mlTimestamp);
            pc->_updateAmclPos();
          }
          if (!is_get_jump) {
            jump_distance = 0;
            ...
            jump_distance +=
              SLAMMath::Dist(last_ladar_pos.mfX, last_ladar_pos.mfY,
                          pc->amcl_current_pos_.mfX, pc->amcl_current_pos_.mfY);
          }
...
void *LocationManagerImpl::_wanJiLocationFunction(void *param) {
  ...
  while (1) {
    ...
    internal_common::RunTime run_time;
    bool is_can_do_location = pc->is_move_ && pc->is_finish_global_location;
    pc->wan_ji_location_->HandleLaserData(
      radar_data, last_pos, is_can_do_location);
...
void LocationManagerImpl::SetQrData(const DMreaderSensoryMessage& data) {
  ...
  if (is_need_qr_location) {
    ...
    if (qr_location_->HandleQrData(data, &qr_pos)) {
      qr_distance_ = 0.0;
      // SLAM_DEBUG("qr get time %ld qr time %ld",
      //             current_time - qr_pos.mlTimestamp, qr_pos.mlTimestamp);
      _updateQrPos(qr_pos);
    }
  }
}
...
void LocationManagerImpl::SetIniPose(const Position &pose,
  bool is_check_result) {
  init_pos_ = pose;
  is_check_result_ = is_check_result;
  init_pos_.mlTimestamp = gomros::common::GetCurrentTime_us();
  grid_location_->SetInitPose(init_pos_.mfX, init_pos_.mfY,
                                   init_pos_.mfTheta);
  qr_location_->SetInitPos(init_pos_);
  Eigen::Matrix3d covariance;
  covariance.setZero(3, 3);
  ekf_for_amcl_->SetInitPos(init_pos_, covariance);
  location_last_pose_ = init_pos_;
  {
    pthread_mutex_lock(&amcl_pos_data_mutex_);
    amcl_current_pos_ = init_pos_;
    amcl_last_pos_ = init_pos_;
    pthread_mutex_unlock(&amcl_pos_data_mutex_);
  }
  {
    std::lock_guard<std::recursive_mutex> lk(reflector_pos_data_mutex_);
    reflector_current_pos_ = init_pos_;
    reflector_last_pos_ = init_pos_;
  }
  SetHavePose(true);
  pthread_mutex_lock(&odom_data_mutex_);
  odom_list_.clear();
...
void LocationManagerImpl::SetOdomData(const OdometerMessage &data) {
  uint64_t last_time = gomros::common::GetCurrentTime_us();
  pthread_mutex_lock(&odom_data_mutex_);
  odom_list_.push_back(data);
  if (gomros::common::GetCurrentTime_us() - last_time > 100000) {
    SLAM_INFO("odom_list_ %ld",
      gomros::common::GetCurrentTime_us() - last_time);
  }
  pthread_mutex_unlock(&odom_data_mutex_);
  pthread_mutex_lock(&last_odom_mutex_);
  last_odom_data_ = data;
  if (gomros::common::GetCurrentTime_us() - last_time > 100000) {
    SLAM_INFO("last_odom_data_ %ld",
      gomros::common::GetCurrentTime_us() - last_time);
  }
  ...
  if (location_config_.location_type == LocationType::FusionLocation ||
    ...
    if (gomros::common::GetCurrentTime_us() - last_time > 100000) {
      SLAM_INFO("odom_list_for_amcl_ %ld",
        gomros::common::GetCurrentTime_us() - last_time);
    }
  ...
  if (location_config_.location_type == LocationType::FusionLocation ||
    ...
    if (gomros::common::GetCurrentTime_us() - last_time > 100000) {
      SLAM_INFO("reflector_location_ %ld",
        gomros::common::GetCurrentTime_us() - last_time);
    }
  }
}
...
gomros::message::Position LocationManagerImpl::GetCurrentPose() {
  ...
  {
    std::lock_guard<std::recursive_mutex> lk(reflector_pos_data_mutex_);
    _calDaltaPos(reflector_last_pos_, current_pos,
                reflector_current_pos_, &land_mark_pos);
  }
  std::map<LocationType, DataFusion::PoseInfo> pos_candidate;
  pos_candidate.insert(std::make_pair(LocationType::AMCL,
                      DataFusion::PoseInfo(
                      Eigen::Vector3f(amcl_pos.mfX, amcl_pos.mfY,
                                      amcl_pos.mfTheta),
                      amcl_pos.mlTimestamp, 1.0, LocationType::AMCL)));
  pos_candidate.insert(std::make_pair(LocationType::QR,
                      DataFusion::PoseInfo(
                      Eigen::Vector3f(current_pos.mfX, current_pos.mfY,
                                      current_pos.mfTheta),
                      current_pos.mlTimestamp, 1.0, LocationType::QR)));
  pos_candidate.insert(std::make_pair(LocationType::LandMarkLocation,
                      DataFusion::PoseInfo(
                      Eigen::Vector3f(land_mark_pos.mfX, land_mark_pos.mfY,
  ...
  float dis = SLAMMath::Dist(result.mfX, result.mfY,
                      location_last_pose_.mfX, location_last_pose_.mfY);
...
LocationManagerImpl::Position LocationManagerImpl::_refreshOdom(
  uint64_t time, bool is_pop) {
  OdometerMessage odom_data;
...
void LocationManagerImpl::_updateAmclPos() {
  ...
  pos_candidate.insert(std::make_pair(LocationType::LandMarkLocation,
                      ...
                      LocationType::LandMarkLocation)));
  DataFusion::PoseInfo result_info;
  location_data_fusion_cal_->GetLocationResult(
    pos_candidate, &result_info);
  Position result;
  result.mfX = result_info.pose_info(0);
  result.mfY = result_info.pose_info(1);
  result.mfTheta = result_info.pose_info(2);
  ...
  uint64_t current_time = gomros::common::GetCurrentTime_us();
  SLAM_INFO("amcl update pose %f %f %f delta time %ld",
            result.mfX, result.mfY, result.mfTheta,
            current_time - result.mlTimestamp);
...
void LocationManagerImpl::_updateQrPos(const Position& qr_pos) {
  ...
  std::map<LocationType, int> types =
    location_data_fusion_cal_->GetLocationResult(
    pos_candidate, &result_info, true);
  Position result;
  result.mfX = result_info.pose_info(0);
  result.mfY = result_info.pose_info(1);
  result.mfTheta = result_info.pose_info(2);
...
void LocationManagerImpl::_calDaltaPos(
  const Position& last_pos, const Position& now_pos,
  ...
  now_odom_pos_mat <<
    cos(now_pos.mfTheta), -sin(now_pos.mfTheta), now_pos.mfX,
    sin(now_pos.mfTheta), cos(now_pos.mfTheta), now_pos.mfY,
    0, 0, 1;
  Eigen::Matrix3d last_odom_pos_mat(3, 3);
  last_odom_pos_mat <<
    cos(last_pos.mfTheta), -sin(last_pos.mfTheta), last_pos.mfX,
    sin(last_pos.mfTheta), cos(last_pos.mfTheta), last_pos.mfY,
    0, 0, 1;
  Eigen::Matrix3d delta_pos_mat(3, 3);
  delta_pos_mat = last_odom_pos_mat.inverse() * now_odom_pos_mat;
  Eigen::Matrix3d amcl_pos_mat(3, 3);
  amcl_pos_mat <<
  cos(now_amcl_pos.mfTheta), -sin(now_amcl_pos.mfTheta), now_amcl_pos.mfX,
  sin(now_amcl_pos.mfTheta), cos(now_amcl_pos.mfTheta), now_amcl_pos.mfY,
  0, 0, 1;
...
bool LocationManagerImpl::_isPosJump(const Position& predictive_pos,
  ...
  if (fabs(predictive_pos.mfX - real_pos.mfX) > 0.1 ||
      ...
      fabs(SLAMMath::NormalizePITheta(
        predictive_pos.mfTheta - real_pos.mfTheta)) > 0.1) {
      SLAM_WARN("position jump %f %f %f -> %f %f %f time %ld",
        predictive_pos.mfX, predictive_pos.mfY, predictive_pos.mfTheta,
        real_pos.mfX, real_pos.mfY, real_pos.mfTheta, real_pos.mlTimestamp);
...
void LocationManagerImpl::_doGlobalLocateMonitor() {
  ...
  if (location_config_.location_type == LocationType::WanJiLandMarkLocation ||
    ...
    // 判断重定位的位置是否和初始位置相差不大
    if (is_check_result_) {
      if (fabs(init_pos_.mfX - locate_pose.mfX) > 0.1 ||
          fabs(init_pos_.mfY - locate_pose.mfY) > 0.1 ||
          fabs(SLAMMath::NormalizePITheta(
            init_pos_.mfTheta - locate_pose.mfTheta)) > 0.1) {
          is_finish_global_location = true;
          SetIniPose(init_pos_);
          grid_location_->StartGlobalLocating(false);
        SLAM_WARN("relocation result is too far from init pose~~~~");
        while (!grid_location_->IsFinishLocate()) {
          usleep(200000);
        }
        grid_location_->GetCurrentPosition(&locate_pose);
