 ...
 * @LastEditors: renjy
 * @LastEditTime: 2023-11-09 16:56:56
 */
...
: LocationInterface(config) {
  ...
  float ladar_theta = location_config_.sensor_mount.radar_position_theta;
  float ladar_pos_x = location_config_.sensor_mount.radar_position_x;
  float ladar_pos_y = location_config_.sensor_mount.radar_position_y;
...
void WanJiLocation::HandleLaserData(const RadarSensoryMessage &data,
  ...
  if (isnan(ladar_pos_x) || isnan(ladar_pos_y) || isnan(ladar_pos_theta)) {
  ...
  if (print_count_++ == 10) {
    print_count_ = 0;
    SLAM_INFO("move_ladar_global_pos %f %f %f, current_pose %f %f %f",
