 ...
 * @LastEditTime: 2023-11-30 11:43:34
 * @Author: lcfc-desktop
 */
...
void ReflectorLocation::HandleLaserData(const RadarSensoryMessage &data,
  ...
  RadarSensoryInfo radar_info = data.mstruRadarMessage;
  ...
  if (!is_move) return;
  if (location_config_.location_type == LocationType::Trilateral) {
    if (!is_move) {
      // 获取静止情况下 激光雷达在全局坐标中的位置
      std::vector<Eigen::Vector2d> obs_centers;
      DataFusion::Observation obs;
      obs.GetObsFromLadarData(location_config_.sensor_mount,
      location_config_.land_mark_config, radar_info);
      ...
      if (reflector_global_cal_->IsGetGlobalPos(
        ...
        transform.SetPose2InPose1(Eigen::Vector3d(
          location_config_.sensor_mount.radar_position_x,
          location_config_.sensor_mount.radar_position_y,
          location_config_.sensor_mount.radar_position_theta));
  ...
  if (!is_move) is_get_location_pos_ = false;
#ifdef TEST_DEBUG
  if (!is_get_location_pos_) return;
  _publishLadarInfoForTest(data.mstruRadarMessage, obs.centers);
#endif
}
...
// #endif
    ...
    //   DataFusion::Observation obs;
    //   obs.GetObsFromLadarData(pgl->location_config_.sensor_mount,
    //       pgl->location_config_.land_mark_config, radar_info[i]);
    //   data_fusion.AddPoint(obs.centers);
    // }
    // data_fusion.GetResult(&obs_centers);
    if (radar_info.size() <= 0) {
      // SLAM_WARN("wating for ladar date ....");
      continue;
    }
    DataFusion::Observation obs;
    obs.GetObsFromLadarData(pgl->location_config_.sensor_mount,
        pgl->location_config_.land_mark_config, radar_info.back());
    obs_centers = obs.centers;
...
void ReflectorLocation::_publishLadarInfoForTest(
  const RadarSensoryInfo& raw_ladar, const std::vector<Eigen::Vector2d>& obs) {
  RadarSensoryInfo ladar_with_pos;
  ladar_with_pos = raw_ladar;
  ladar_with_pos.mstruRadarHeaderData.mfXPos = current_pose_.mfX;
  ladar_with_pos.mstruRadarHeaderData.mfYPos = current_pose_.mfY;
  ladar_with_pos.mstruRadarHeaderData.mfTheta = current_pose_.mfTheta;
  ladar_with_pos.mstruRadarHeaderData.mcPosValid = 1;
  ...
  display_location_result_->DisPlayReflectorLocation(ladar_with_pos,
                  obs, match_obs, current_cov_);
  return;
}
