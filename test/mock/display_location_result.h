 ...
 * @LastEditors: renjy
 * @LastEditTime: 2023-11-18 15:11:56
 */
...
class LocationResult {
 ...
 private:
  ...
  /**
   * @description: 实时改变地图信息
   * @param {float} x
   * @param {float} y
   * @return {*}
   */  
  void _changeMapInfo(float x, float y);
  /**
  ...
  /**
   * @description: 加载反光柱建图线程
   * @return {*}
   */
  static void *_displayReflectorMappingPthread(void *param);
  /**
   * @description:  反光柱建图信息
   * @return {*}
   */
  static void _ReflectorMappingEvent(void *object, char *buf, const int size);
   ...
   * @return {*}
   */
  bool _reflectorMapping(const RadarSensoryMessage& ladar_info, cv::Mat* map);


  /**
   * @description: 加载反光柱定位线程
   * @return {*}
   */
  static void *_displayReflectorLocationPthread(void *param);

  /**
   * @description:  反光柱定位信息
   * @return {*}
   */
  static void _ReflectorLocationEvent(void *object, char *buf, const int size);
   ...
   * @return {*}
   */
  bool _reflectorLocation(const RadarSensoryMessage& ladar_info, cv::Mat* map);
 ...
 private:
  ...
  // 雷达原始数据及雷达在全局中的坐标  -- 栅格定位
  std::list<RadarSensoryMessage> raw_ladar_datas_;

  pthread_mutex_t reflector_mapping_mutex_;
  std::list<RadarSensoryMessage> reflector_mapping_datas_;

  pthread_mutex_t reflector_location_mutex_;
  std::list<RadarSensoryMessage> reflector_location_datas_;


  pthread_t display_ladar_pthread;
  pthread_t display_map_pthread;
  pthread_t display_reflector_mapping_pthread;
  pthread_t display_reflector_location_pthread;
