 ...
 * @LastEditTime: 2023-11-29 21:00:40
 */
#include <float.h>
#include <pthread.h>
#include <fstream>
...
bool SensorData::_getLadarMessage(
  ...
  message->mstruRadarMessage.mstruRadarHeaderData.mfAngleMax =
    config_.mapping_config.mapping_end_angle * M_PI / 180.0f;
  message->mstruRadarMessage.mstruRadarHeaderData.mfAngleMin =
    config_.mapping_config.mapping_start_angle * M_PI / 180.0f;
  ...
  snprintf(open_file_name, sizeof(open_file_name), \
            "./out/ladar_data_%02d.pcd", file_count++);
  ...
  sensor_mount.radar_position_x = 0.0;
  sensor_mount.radar_position_y = 0.0;
  sensor_mount.radar_position_theta = 0.0;

  obs.GetObsFromLadarData(sensor_mount,
                          config_.mapping_config.land_mark_config,
  ...
  for (auto iter : obs.centers) {
    for (float i = -M_PI; i < M_PI; i += (M_PI / 180.f)) {
      pcl::PointXYZI point;
      point.x = iter.x() + r * std::cos(i);
      point.y = iter.y() + r * std::sin(i);;
      point.z = 1.0;
      point.intensity = 2000;
      cloud_in.push_back(point);
    }
  }
  if (!cloud_in.empty())
    pcl::io::savePCDFileASCII<pcl::PointXYZI> (open_file_name, cloud_in);
  return true;
}
